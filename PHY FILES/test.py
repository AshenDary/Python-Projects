...
class TrainFareCalculator:
    ...
    def create_widgets(self):
        font = ("Helvetica", 16)  # Set a larger font size
        button_font = ("Helvetica", 16, "bold")  # Font for the button

        main_frame = tk.Frame(self.root, bg="#3a3f4b", bd=5)
        main_frame.pack(pady=30, padx=30, fill="both", expand=True)

        title_label = tk.Label(main_frame, text="LRT AND MRT", font=("Helvetica", 24, "bold"), fg="white", bg="#3a3f4b")
        title_label.pack(pady=(0, 20))

        # Card and Passenger Type Selection
        ...
        card_type_frame = tk.Frame(main_frame, bg="#3a3f4b")
        card_type_frame.pack(pady=5)
        ttk.Label(card_type_frame, text="Select Card Type:", font=font).pack(side=tk.LEFT, padx=10)
        ...
        passenger_type_frame = tk.Frame(main_frame, bg="#3a3f4b")
        passenger_type_frame.pack(pady=5)
        ttk.Label(passenger_type_frame, text="Select Passenger Type:", font=font).pack(side=tk.LEFT, padx=10)
        ...
        terminal_frame = tk.Frame(main_frame, bg="#3a3f4b")
        terminal_frame.pack(pady=5)
        ttk.Label(terminal_frame, text="Select Terminal:", font=font).pack(side=tk.LEFT, padx=10)
        ...
        start_station_frame = tk.Frame(main_frame, bg="#3a3f4b")
        start_station_frame.pack(pady=5)
        ttk.Label(start_station_frame, text="Select Start Station:", font=font).pack(side=tk.LEFT, padx=10)
        ...
        end_station_frame = tk.Frame(main_frame, bg="#3a3f4b")
        end_station_frame.pack(pady=5)
        ttk.Label(end_station_frame, text="Select End Station:", font=font).pack(side=tk.LEFT, padx=10)
        ...
        # Timer Label
        self.timer_label = ttk.Label(main_frame, text="Time left: 5 seconds", font=font)
        self.timer_label.pack(pady=5)

        # Fine Notification Label
        self.fine_notification_label = ttk.Label(main_frame, text="", foreground="red", font=font)
        self.fine_notification_label.pack(pady=5)

        # Calculate Fare Button
        self.calculate_button = tk.Button(main_frame, text="Calculate Fare", command=self.calculate_fare_gui, font=button_font)
        self.calculate_button.pack(pady=10)

        # Result and Transfer Prompt
        self.result_label = ttk.Label(main_frame, text="", wraplength=300, font=font)
        self.result_label.pack(pady=5)

        self.transfer_prompt_label = ttk.Label(main_frame, text="", wraplength=300, font=font)
        self.transfer_prompt_label.pack(pady=5)

        self.transfer_button_frame = tk.Frame(main_frame, bg="#3a3f4b")
        self.transfer_button_frame.pack(pady=5)

        self.yes_button = ttk.Button(self.transfer_button_frame, text="Transfer", command=self.switch_lines, style="TButton")
        self.continue_button = ttk.Button(self.transfer_button_frame, text="Continue", command=self.continue_travel, style="TButton")
        self.no_button = ttk.Button(self.transfer_button_frame, text="No", command=self.end_travel, style="TButton")
    ...
    def update_stations(self, event):
        ...
        if terminal == "LRT1":
            stations = ["Station1", "Station2"]  # Example stations
        elif terminal == "LRT2":
            stations = ["Station3", "Station4"]  # Example stations
        elif terminal == "MRT3":
            stations = ["Station5", "Station6"]  # Example stations
        else:
            stations = []
    ...
    def end_travel(self):
        self.result_label.config(text=f"Total distance traveled: {self.total_distance:.2f} km\nTotal fare for all rides: ₱{self.total_fare:.2f}")
        self.transfer_prompt_label.config(text="")
        self.transfer_button_frame.pack_forget()
