...
style = ttk.Style()
style.configure('TLabel', background='pink', font=('Lucida Console', 10))
style.configure('TButton', background='pink', font=('Lucida Console', 10))
style.configure('TCombobox', font=('Lucida Console', 10))
...
reset_program_btn = tk.Button(root, text="Reset Program", bg="pink", font=('Lucida Console', 10), command=lambda: reset_program())
...
def handle_transfer(transfer):
    ...
    if transfer:
        ...
        start_station.set(end_station.get())  # Set start station to previous end station
        end_station.set('')  # Clear end station for a new selection
        start_timer()  # Continue the timer
...
def start_timer():
    ...
    global start_time, fine_applied, timer_paused, timer_running
    fine_applied = False
    timer_paused = False
    timer_running = True
...
def update_timer_display():
    ...
    if timer_stopped or not timer_running:
        return  # Exit the function if the timer is stopped due to insufficient balance or user action
    ...
    if start_time:
        ...
        global timer_id
        timer_id = root.after(100, update_timer_display)
...
continue_riding_label = tk.Label(root, text="", bg='pink', font=('Lucida Console', 10))
...
yes_button = tk.Button(root, text="Yes", command=continue_ride, bg='pink', font=('Lucida Console', 10))
no_button = tk.Button(root, text="No", command=end_ride, bg='pink', font=('Lucida Console', 10))
...
def calculate_fare():
    ...
    # Calculate and accumulate the total distance traveled
    ...
    # Calculate and accumulate the total stations passed
...
calculate_btn = tk.Button(root, text="Calculate Fare", command=calculate_fare, bg='pink', font=('Lucida Console', 10))
